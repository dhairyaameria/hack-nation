...
**Depends on:** API contracts in `01-CONTRACTS.md` (build against mocked responses until real endpoints land).
...
4. **VC Copilot chat panel** (persistent, e.g. right-side drawer):
   - Free-form questions → answers showing which skill(s) ran + inline citations.
...
- [ ] Copilot panel shows skill names + citations + re-run diff.
